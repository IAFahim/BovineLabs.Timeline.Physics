...
namespace BovineLabs.Timeline.Physics.Authoring.Editor
{
    ...
    public class PhysicsAngularPIDClipEditor : UnityEditor.Editor
    {
        ...
        {
            var clip = (PhysicsAngularPIDClip)target;
            ...
            else
            {
                ...
                EditorGUILayout.PropertyField(serializedObject.FindProperty(nameof(PhysicsAngularPIDClip.chaseTargetBlend)));
                EditorGUILayout.PropertyField(serializedObject.FindProperty(nameof(PhysicsAngularPIDClip.localTargetRotationEuler)));
                EditorGUILayout.PropertyField(serializedObject.FindProperty(nameof(PhysicsAngularPIDClip.maxTorque)));
                
                EditorGUILayout.Space(2);
                EditorGUILayout.LabelField("Gains", EditorStyles.boldLabel);
                EditorGUILayout.PropertyField(serializedObject.FindProperty(nameof(PhysicsAngularPIDClip.uniformAxes)));
                DrawPIDField(clip, nameof(PhysicsAngularPIDClip.proportional), "Proportional (P)");
                DrawPIDField(clip, nameof(PhysicsAngularPIDClip.integral),     "Integral (I)");
                DrawPIDField(clip, nameof(PhysicsAngularPIDClip.derivative),   "Derivative (D)");
            }

            serializedObject.ApplyModifiedProperties();
        }

        private void DrawPresets(PhysicsAngularPIDClip clip)
        {
            var cols = 2;
        ...
        private void DrawTuningRows(PhysicsAngularPIDClip clip)
        {
        ...
        private void DrawTuningRow(PhysicsAngularPIDClip clip, string label, string tooltip, float p, float i, float d)
        {
        ...
        private void DrawPIDField(PhysicsAngularPIDClip clip, string propName, string displayName)
        {
