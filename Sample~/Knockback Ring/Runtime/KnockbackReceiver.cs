namespace Vex.Knockback
{
    ...
    /// <see cref="DirectionalKnockbackSystem"/> appends a one-shot <c>PendingExternalForce</c> impulse pointing away —
    /// on the XZ plane — from the body that entered, plus a vertical "leap" component. It rides the external
    /// (knockback) velocity channel, so a braking/drag clip on the victim can't cancel the hit. Direction is derived from the contact
